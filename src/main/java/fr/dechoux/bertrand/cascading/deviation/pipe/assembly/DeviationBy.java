...
/**
 * Class DeviationBy is used to average and compute the standard deviation of
 * values associated with duplicate keys in a tuple stream.
 * <p/>
 * This strategy based on {@link AggregateBy} is similar to using
 * {@code combiners}, except no sorting or serialization is invoked and results
 * in a much simpler mechanism.
 * <p/>
 * The {@code threshold} value tells the underlying partials functions how many
 * unique key sums and counts to accumulate in the LRU cache, before emitting
...
public class DeviationBy extends AggregateBy {
	...
	private static final Fields BIND_FIELDS = new Fields( //
			DeviationBy.class.getPackage().getName() + ".sum",//
			DeviationBy.class.getPackage().getName() + ".sumOfSquare",//
			DeviationBy.class.getPackage().getName() + ".count");

	/**
	 * Class DeviationPartials is a
	 * {@link cascading.pipe.assembly.AggregateBy.Functor} that is used to
	 * square, count and sum observed duplicates from the tuple stream.
	 * 
	...
	public static class DeviationPartials implements Functor {
		...
		/**
		 * Constructor DeviationPartials creates a new DeviationPartials
		 * instance.
		 */
	...
	/**
	 * Class DeviationFinal is used to finalize the operation on the Reduce side
	 * of the process. It must be used in tandem with a
	 * {@link DeviationPartials} Functor.
	 */
	public static class DeviationFinal extends BaseOperation<DeviationFinal.Context> implements
		...
		/**
		 * Constructs a new instance that returns the aggregates of the values
		 * encountered in the given fieldDeclaration field name.
		 */
		public DeviationFinal(Fields fieldDeclaration) {
			super(2, fieldDeclaration);

			if (!fieldDeclaration.isSubstitution() && fieldDeclaration.size() != 2)
				throw new IllegalArgumentException("fieldDeclaration may only declare 2 fields, got: "
	...
	/**
	 * Use this constructor when used with a
	 * {@link cascading.pipe.assembly.AggregateBy} instance.
	 */
